<template>
  <div class="mContainer">
    ...
    <div class="mtools">
      <el-form label-width="90px" size="small" :inline="true">
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="newLedger()">新建代理合同</el-button>
        </el-form-item>
      </el-form>
    </div>
    <m-table
    ...
    >
      <el-table-column label="操作" width="300px" fixed="right">
        <template slot-scope="scope">
          <el-button type="text" 
            style="color: red" 
            @click="toDelate(scope.row.agencyId)"
          ...
          <el-button 
            type="text" 
            @click="showChangeContract(scope.row.agencyId)"
            v-if="scope.row.status!==0"
            :disabled="(scope.row.isBroker !== 0) || (scope.row.isShopowner !== 0) || (scope.row.isManager !== 0) || (scope.row.isAudit !== 0)"
            >变更合同</el-button>
          <el-button type="text" 
    ...
    <!-- 变更合同changeContract-->
    <change-contract v-if="isShowChangeContract" ref="changeContract" :id="activeChangeContractId" @update="currentChange" @close="closeChangeContract"></change-contract>
    <!-- 变更合同END -->
    ...
    >
      <el-form label-width="70px" size="small">
        <el-form-item label="作废原因">
          <el-input v-model="stopReson" type="textarea" :rows="5" size="small" style="width: 350px;"></el-input>
        </el-form-item>
      </el-form>
    ...
    <!-- 新建/修改代理合同（台账，业主） -->
    <m-detail v-if="visible" :visible.sync="visible" :type="detailType" :agencyNo="agencyNo" :id="activeChangeContractId" @update="currentChange" @close="closeContract"></m-detail>
    <!-- 新建/修改代理合同END -->
    ...
    >
      <el-form label-width="70px" size="small">
        <el-form-item label="类型">
          <el-input class="minput" value="预付" style="width: 220px;" disabled="true"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      stopReson: '', // 作废原因
      stopAgencyId: '',// 作废合同id
      activeChangeContractId: '', // 当前选择的代理合同id
      agencyNo: '',
  ...
  methods: {
    ...
    */
    dblhandleCurrentChange(ele){
      ...
      this.activeChangeContractId = ele.agencyId
      this.agencyNo = ele.agencyNo
      this.visible = false;
      if (!this.visible) {
        this.visible = true;
      }
    ...
    handleCurrentChange(val){ //单选
      if(!val) return
      findByAgencyNo(val.agencySysNo).then(res => {
        let { code, body } = res;
        this.approve = val
        if (body) {
    ...
    currentChange(val, totalNum) {
      ...
      getAgencyByPage({ ...parms, page, rows }).then(res => {
        let { code, body } = res;
        if (code == 1000) {
          ...
          this.$nextTick(() => {
            ...
            this.$refs.mtable.setCurrentRow(this.table.list.find(d => d.agencyId === this.currentRow.agencyId))
          })
        }else {
          this.table.total = 0
        }
    ...
    toStop(ele) {
      ...
      this.stopAgencyId = ele.agencyId
      this.agencyNo = ele.agencyNo
    },
    /**
     * 提交作废
    ...
    submitStop() {
      ...
      cancelAgency({agencyId: this.stopAgencyId, agencyNo: this.agencyNo, reason: this.stopReson, status: 0}).then(res => {
          if(res.code == 1000) {
            this.$message({
              type: "success",
              message: "作废成功!"
    ...
    toDelate(agencyId) {
      this.$confirm("此操作将删除该合同, 是否继续?", "提示", {
        ...
        deleteAgency({agencyId}).then((res) => {
          if(res.code == 1000) {
            this.$message({
              type: "success",
              message: "删除成功!"
          ...
          switch (type) {
            case 1:
              const params1 = {
                ...
                agencyIds: [this.approve.agencyId]
              };
              // 请使用合同支出的审核接口（下同）
