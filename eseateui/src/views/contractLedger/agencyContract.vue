...
import {
  getAgencyByPage,getContractDetail
...
export default {
  ...
  methods: {
     ...
     */
    getAgencys(params = {}) {
      getAgencyByPage(params).then(res => {
        let { code, body } = res;
        if (code == 1000) {
          //合同列表
    ...
    getContract(agencyId) {
      getContractDetail(agencyId).then(res=>{
        let { code, body } = res;
        if(code == 1000){
          this.table2.list = body.agencyPaymentAgreementVOS || [];
          this.table3.list = body.agencyCostDetailVOList || [];
          return
        }
    ...
    changeAgency(agency) {
      ...
      this.getContract(agency.agencyId)
    },
    /**
     * 账户余额求累加和
    */
