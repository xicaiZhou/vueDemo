  ...
  >
    ...
    >
      ...
      <el-form-item label="客户分类">
        ...
        <!-- <el-input v-model="addOwnerData.classification" class="minput"></el-input> -->
      </el-form-item>
...
import { createMember, findMemberPager } from "@/api/org-member";
...
export default {
  ...
  data() {
    return {
      ...
      types: [{ value: 1, label: "客户" }, { value: 2, label: "业主" }],
      classifications: [
        { value: 1, label: "个人" },
        { value: 2, label: "单位" },
        { value: 3, label: "国外" }
  ...
  methods: {
    ...
    checkIdcard() {
      ...
      return findMemberPager({ identity }).then(res => {
        console.log(res)
        const isExist = res.body.length > 0
        if (isExist) {
          this.$message({
            type: "warning",
            message: "身份证号已存在!"
